...
var getServers = function(app, zk, servers) {
	...
	var latch = countDownLatch.createCountDownLatch(servers.length, {timeout: 1000 * 60}, function() {
		...
		logger.error('%s get servers by zookeeper servers number : %j', app.serverId, utils.size(results));
		logger.error('%s get servers by zookeeper servers : %j', app.serverId, results);
	...
	for(var i = 0; i < servers.length; i++) {
		(function(index) {
			zk.getData(zk.path + '/' + servers[index], function(err, data) {
				if(!!err)	{
					logger.error('get data failed for server : %s', app.serverId);
					latch.done();
					success = false;
					return;
				}
